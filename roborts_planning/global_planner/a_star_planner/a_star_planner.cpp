...
AStarPlanner::AStarPlanner(CostmapPtr costmap_ptr)
  ...
  clean_client_ =
      nh_.serviceClient<roborts_msgs::CleanCellInfos>("/clean_cell_infos");
  update_client_ =
      nh_.serviceClient<roborts_msgs::UpdateCellInfos>("/update_cell_infos");
  get_client_ =
      nh_.serviceClient<roborts_msgs::GetCellInfos>("/get_cell_infos");
  get_index_client_ =
      nh_.serviceClient<roborts_msgs::GetCurrentIndex>("/get_current_index");
  get_path_client_ =
      nh_.serviceClient<roborts_msgs::GetCurrentPath>("/get_current_path");
}
...
ErrorInfo AStarPlanner::Plan(const geometry_msgs::PoseStamped &start,
  ...
  if (costmap_ptr_->GetCostMap()->GetCost(goal_x, goal_y) <
    ...
    while (tmp_goal_y <= goal_y + goal_search_tolerance_) {
      ...
      while (tmp_goal_x <= goal_x + goal_search_tolerance_) {
        ...
        unsigned int dist = abs(static_cast<long>(goal_x) - tmp_goal_x) +
                            abs(static_cast<long>(goal_y) - tmp_goal_y);
  ...
  if (!goal_valid) {
    ...
    if (start_index == goal_index) {
      ...
      error_info = SearchPath(start_index, goal_index, path, robot_id,
                              syne_robot_id, is_clean);
      if (error_info.IsOK()) {
...
ErrorInfo AStarPlanner::SearchPath(
    ...
    std::vector<geometry_msgs::PoseStamped> &path, const std::string &robot_id,
    const std::string &syne_robot_id, const bool is_clean) {
  g_score_.clear();
  f_score_.clear();
  parent_.clear();
  state_.clear();
  clean_srv_.request.robot_id = robot_id;
  clean_client_.call(clean_srv_);
  path_info_.clear();
  cell_infos_.clear();
  gridmap_width_ = costmap_ptr_->GetCostMap()->GetSizeXCell();
  gridmap_height_ = costmap_ptr_->GetCostMap()->GetSizeYCell();
  ROS_INFO("Search in a map width: %d height: %d.", gridmap_width_,
           gridmap_height_);
  cost_ = costmap_ptr_->GetCostMap()->GetCharMap();
  g_score_.resize(
      gridmap_height_ * gridmap_width_,
      std::numeric_limits<int>::max());
  f_score_.resize(gridmap_height_ * gridmap_width_,
                  std::numeric_limits<int>::max());
  parent_.resize(gridmap_height_ * gridmap_width_,
                 std::numeric_limits<int>::max());
  state_.resize(gridmap_height_ * gridmap_width_,
                SearchState::NOT_HANDLED);
  cell_infos_.resize(gridmap_height_ * gridmap_width_);
  get_client_.call(get_srv_);
  cell_infos_ = get_srv_.response.cell_infos;
  // Get Syne Robot Path
  get_path_srv_.request.robot_id = syne_robot_id;
  get_path_client_.call(get_path_srv_);
  syne_path_ = get_path_srv_.response.path_info;
  ROS_INFO("syne:size:%d", syne_path_.size());
  ...
  int current_index, move_cost, h_score, count = 0, syne_index{-1};
  int handled_index = GetHandledIndex(syne_robot_id);

  geometry_msgs::PoseStamped syne_robot_pose;
  unsigned int syne_robot_x, syne_robot_y;
  if (costmap_ptr_->GetRobotPose(syne_robot_id + "/base_link",
                                 syne_robot_pose)) {
  ...
  while (!openlist.empty()) {
    ...
    for (auto neighbor_index : neighbors_index) {
      ...
      if ((cost_[neighbor_index] >= inaccessible_cost_ &&
           (!is_clean ||
            GetEuclideanDistance(neighbor_index, start_index) > 50)) ||
      ...
      if ((handled_index != -1 && !cell_infos.empty() &&
        ...
        continue;
      } else if (!CheckPath(handled_index, neighbor_index)) {
        move_cost += 200;
      } else if (GetEuclideanDistance(neighbor_index, syne_index) < 100) {
        move_cost += 400;
      }
  ...
  cell_info_.cost_g = g_score_.at(iter_index);
  cell_info_.index = iter_index;
  path_info_.push_back(cell_info_);
  ...
  update_srv_.request.robot_id = robot_id;
  update_srv_.request.cell_infos = path_info_;
  update_client_.call(update_srv_);
...
ErrorInfo AStarPlanner::GetMoveCost(const int &current_index,
  ...
  if (abs(neighbor_index - current_index) == 1 ||
      abs(neighbor_index - current_index) == gridmap_width_) {  // kunkun
    move_cost = 10;
...
void AStarPlanner::GetManhattanDistance(const int &index1, const int &index2,
  ...
  manhattan_distance = heuristic_factor_ * 10 *
                       (abs(static_cast<long>(index1) / gridmap_width_ -
...
int AStarPlanner::GetHandledIndex(const std::string &robot_id) {
  get_index_srv_.request.robot_id = robot_id;
  get_index_client_.call(get_index_srv_);
  return get_index_srv_.response.result;
}

double AStarPlanner::GetEuclideanDistance(const int &my_index,
                                          const int &syne_index) {
  int x = abs(static_cast<long>(my_index) / gridmap_width_ -
              syne_index / gridmap_width_);
  int y = abs(static_cast<long>(my_index) % gridmap_width_ -
              syne_index % gridmap_width_);
  return sqrt(x * x + y * y);
}

bool AStarPlanner::CheckPath(const int &handled_index,
                             const int &current_index) {
  for (int i = syne_path_.size() - 1; i >= 0; i--) {
    if (GetEuclideanDistance(current_index, syne_path_.at(i).index) < 50) {
      return false;
    }
  }
  return true;
